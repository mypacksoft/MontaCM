/*
  # YB Manager - Initial Schema

  Creates all tables needed for the YugabyteDB infrastructure management platform.

  1. New Tables
    - physical_hosts: Hyper-V host machines with agent connection details (ip, port, api_key)
    - vms: Virtual machines managed on physical hosts
    - yugabyte_clusters: YugabyteDB cluster definitions
    - yugabyte_nodes: Individual node memberships within clusters
    - automation_jobs: Job queue for async operations (test_host, create_vm, etc.)
    - job_steps: Step-level tracking within jobs
    - job_logs: Structured log lines per job
    - system_config: Key-value configuration store

  2. Security
    - RLS enabled on all tables
    - Policies allow full access to anon role (internal tool, no auth required)

  3. Other
    - Triggers for updated_at timestamps
    - pg_notify trigger for new automation jobs (consumed by Python daemon)
    - Seed data for system_config defaults
*/
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE constraint_name = 'fk_vms_cluster'
  ) THEN
...
-- RLS
ALTER TABLE physical_hosts ENABLE ROW LEVEL SECURITY;
ALTER TABLE vms ENABLE ROW LEVEL SECURITY;
ALTER TABLE yugabyte_clusters ENABLE ROW LEVEL SECURITY;
ALTER TABLE yugabyte_nodes ENABLE ROW LEVEL SECURITY;
ALTER TABLE automation_jobs ENABLE ROW LEVEL SECURITY;
ALTER TABLE job_steps ENABLE ROW LEVEL SECURITY;
ALTER TABLE job_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE system_config ENABLE ROW LEVEL SECURITY;

-- RLS Policies (anon access for internal tool)
CREATE POLICY "anon can select physical_hosts"
  ON physical_hosts FOR SELECT TO anon USING (true);
CREATE POLICY "anon can insert physical_hosts"
  ON physical_hosts FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon can update physical_hosts"
  ON physical_hosts FOR UPDATE TO anon USING (true) WITH CHECK (true);
CREATE POLICY "anon can delete physical_hosts"
  ON physical_hosts FOR DELETE TO anon USING (true);

CREATE POLICY "anon can select vms"
  ON vms FOR SELECT TO anon USING (true);
CREATE POLICY "anon can insert vms"
  ON vms FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon can update vms"
  ON vms FOR UPDATE TO anon USING (true) WITH CHECK (true);
CREATE POLICY "anon can delete vms"
  ON vms FOR DELETE TO anon USING (true);

CREATE POLICY "anon can select yugabyte_clusters"
  ON yugabyte_clusters FOR SELECT TO anon USING (true);
CREATE POLICY "anon can insert yugabyte_clusters"
  ON yugabyte_clusters FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon can update yugabyte_clusters"
  ON yugabyte_clusters FOR UPDATE TO anon USING (true) WITH CHECK (true);
CREATE POLICY "anon can delete yugabyte_clusters"
  ON yugabyte_clusters FOR DELETE TO anon USING (true);

CREATE POLICY "anon can select yugabyte_nodes"
  ON yugabyte_nodes FOR SELECT TO anon USING (true);
CREATE POLICY "anon can insert yugabyte_nodes"
  ON yugabyte_nodes FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon can update yugabyte_nodes"
  ON yugabyte_nodes FOR UPDATE TO anon USING (true) WITH CHECK (true);
CREATE POLICY "anon can delete yugabyte_nodes"
  ON yugabyte_nodes FOR DELETE TO anon USING (true);

CREATE POLICY "anon can select automation_jobs"
  ON automation_jobs FOR SELECT TO anon USING (true);
CREATE POLICY "anon can insert automation_jobs"
  ON automation_jobs FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon can update automation_jobs"
  ON automation_jobs FOR UPDATE TO anon USING (true) WITH CHECK (true);
CREATE POLICY "anon can delete automation_jobs"
  ON automation_jobs FOR DELETE TO anon USING (true);

CREATE POLICY "anon can select job_steps"
  ON job_steps FOR SELECT TO anon USING (true);
CREATE POLICY "anon can insert job_steps"
  ON job_steps FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon can update job_steps"
  ON job_steps FOR UPDATE TO anon USING (true) WITH CHECK (true);

CREATE POLICY "anon can select job_logs"
  ON job_logs FOR SELECT TO anon USING (true);
CREATE POLICY "anon can insert job_logs"
  ON job_logs FOR INSERT TO anon WITH CHECK (true);

CREATE POLICY "anon can select system_config"
...
-- Trigger: notify daemon of new jobs
