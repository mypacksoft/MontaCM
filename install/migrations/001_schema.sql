...
INSERT INTO system_config (key, value, description) VALUES
  ...
  ('ubuntu_iso_path',               '',                   'Full path to Ubuntu live-server ISO on the Hyper-V host, e.g. E:\YSDB\ubuntu-24.04.4-live-server-amd64.iso'),
  ('default_ssh_user',              'ubuntu',             'Default OS user created on provisioned VMs'),
  ('default_ssh_public_key',        '',                   'SSH public key injected into new VMs via autoinstall'),
  ('default_user_password_hash',    '',                   'SHA-512 password hash for the default user (generate with: python3 -c "import crypt; print(crypt.crypt(\"password\", crypt.mksalt(crypt.METHOD_SHA512)))")'),
  ('dns_server',                    '8.8.8.8',            'DNS server IP for VM network configuration'),
